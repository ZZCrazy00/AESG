...
from model import DeepAE1, DeepAE2
...
class MyDataset(Dataset):
    ...
    def process_data(self, args):
        drug = np.loadtxt("dataset/{}/drugsimilarity.txt".format(args.dataset_name))  # MDAD 1373 aBiofilm 1720 DrugVirus 175
        microbe = np.loadtxt("dataset/{}/microbesimilarity.txt".format(args.dataset_name))  # 173 140 95
        adj = np.loadtxt("dataset/{}/adj.txt".format(args.dataset_name))  # 2470
        _, drug_emb = DeepAE1(drug, args)
        _, microbe_emb = DeepAE2(microbe, args)
        adj = adj[:, :-1]
        ...
        return Data(x=feature, edge_index=adj)
